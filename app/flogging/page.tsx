...
export default function FloggingPage() {
  ...
  // Redirect to game if reveal is received
  useEffect(() => {
    if (floggingReveal) {
      router.push("/game");
    } else if (lobby && lobby.status !== "PLAYING") {
      router.push("/lobby");
    } else if (lobby?.isFloggingUsed) {
      window.alert("Flogging has already been used this game.");
      router.push("/game");
    } else if (!lobby) {
      // router.push("/"); // Let loading state handle it or redirect if timeout
    }
  }, [lobby, router, floggingReveal]);
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-8 animate-in zoom-in-95 duration-300 flex flex-col items-center mx-4">
              ...
              <p className="text-slate-400 text-center">
                The sailor must accept their punishment...
              </p>
            </div>
          </div>
        )}
