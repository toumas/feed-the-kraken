    ...
    RoleReveal: {
      Root: ({
        ...
        <div data-testid="role-reveal-root">
          <button type="button" onClick={onReveal} data-testid="trigger-reveal">
            Trigger Reveal
          </button>
          {children}
        </div>
      ...
      Revealed: ({ children }: { children: React.ReactNode }) => (
        <div>{children}</div>
      ),
...
vi.mock("../../i18n/client", () => ({
  useT: (_ns: string) => ({
    t: (key: string, options?: { name?: string }) => {
      ...
      if (key === "cabinSearch.searchedDesc")
        return "Tap 5 times to reveal the loyalty card.";
      if (key === "roleReveal.instruction")
        return "Tap 5 times to reveal your role.";
      if (key === "cabinSearch.foundCard")
        return `You found ${options?.name}'s loyalty card!`;
      if (key === "cabinSearch.confirmCloseWithoutReveal")
...
describe("CabinSearchView", () => {
  ...
  it("renders the correct instruction text when showing search results", () => {
    ...
    expect(
      screen.getByText("Tap 5 times to reveal the loyalty card."),
    ).toBeDefined();
  ...
  describe("Confirmation Dialog", () => {
    ...
    it("prompts for confirmation if closing without revealing", () => {
      ...
      expect(screen.getByText("Confirm close?")).toBeDefined();
